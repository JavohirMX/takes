...
enum PieceDetection {
    ...
    /// Highest-confidence piece label per square from padded-warp boxes.
    static func classes(
        from boxes: [Box],
        paddedImageSize: CGFloat,
        margin: CGFloat,
        ...
        for box in boxes {
            ...
            candidates.append(
                Candidate(square: square, piece: box.piece, confidence: box.confidence)
            )
        }
        return classes(from: candidates)
    }
